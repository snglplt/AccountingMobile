...
import com.selpar.selparbulut.adapter.GidenFaturaAdapter
...
class GidenFaturaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_giden_fatura)
        onBaslat()
    ...
    private fun onFaturaGetir() {
        if(entegrator=="EFINANS"){
            ...
            var url = "https://pratikhasar.com/finans/e_giden_listesi_finans.php?"+urlsb
            Log.d("giden_park_listesi", url)
            ...
            val request = JsonObjectRequest(
                ...
                { response ->
                    try {
                        val json = response["finans_giden_faturalar"] as JSONArray
                        val itemList: ArrayList<GelenFaturaModel> = ArrayList()
            ...
            var url = "https://pratikhasar.com/park/e_giden_listesi_park.php?"+urlsb
            Log.d("giden_park_listesi", url)
            val queue: RequestQueue = Volley.newRequestQueue(this)
            //val requestBody = "tur=deneme"
            val request = JsonObjectRequest(
                ...
                { response ->
                    try {
                        val json = response["park_giden_faturalar"] as JSONArray
                        val itemList: ArrayList<GelenFaturaModel> = ArrayList()
                        for (i in 0 until json.length()) {
                            val item = json.getJSONObject(i)
            ...
            var url = "https://pratikhasar.com/netting/e_giden_listesi_selpar.php?" + urlsb
            Log.d("RESİMYOL", url)
            val queue: RequestQueue = Volley.newRequestQueue(this)
            //val requestBody = "tur=deneme"
            val request = JsonObjectRequest(
    ...
    private fun onPdfGoster(id: String, fatura_id: String) {
        if(entegrator=="EFINANS"){
            ...
            i.putExtra("path","https://pratikhasar.com/finans/e_giden_pdf_finans.php?tip=EFATURA&vergino="+vergino+"&InvoiceUUID="
            ...
            i.putExtra("path","https://pratikhasar.com/park/e_giden_pdf_park.php?InvoiceUUID="
                    +id+"&id="+fatura_id+"&ekullanici="+entegratorkullanici+"&esifre="+entegratorsifre)
            startActivity(i)
        }else {
            val i= Intent(this, FaturaPdfGosterActivity::class.java)
            i.putExtra("path","https://pratikhasar.com/netting/e_giden_pdf.php?firma_id="+"1"+"&InvoiceUUID="+id+"&id="+fatura_id)
            startActivity(i)
        }
