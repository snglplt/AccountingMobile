...
import com.selpar.selparbulut.adapter.KartAcikAdapter
...
class AcikKartlarGercekFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view=inflater.inflate(R.layout.fragment_acik_kartlar_gercek, container, false)
        progressBar = view.findViewById(R.id.progressBar)
        Toast.makeText(requireContext(),getString(R.string.acikkartlarlisteleniyor),Toast.LENGTH_LONG).show()
        ...
        btn_bilgi_ara=view.findViewById(R.id.btn_bilgi_ara)
       // btn_bilgi_ara.setOnClickListener { acikKartAra(edarama.getText().toString()) }
        ...
        txtarac_no.setOnKeyListener(View.OnKeyListener { _, keyCode, event ->
            if (event.action == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_ENTER) {
                ...
                // ...
                acikKartAra(txtarac_no.getText().toString())
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun acikKartAra(arama: String) {
        if(arama.isNotEmpty() && arama.length>=3){
            kartArananGetir(arama)
        }else{
            Toast.makeText(requireContext(),getString(R.string.lutfenaracbilgisigiriniz),Toast.LENGTH_LONG).show()
        }
    }
    @RequiresApi(Build.VERSION_CODES.O)
    private fun kartArananGetir(aranan: String)  {
        ...
        val urlek ="&username="+username+"&uid="+uid+"&search="+aranan+"&baslangic_Tarihi="+today+"&bitis_Tarihi="+fiveDaysAgo+"&kabuldurumu=AÇIK"
        ...
        val request = JsonObjectRequest(
            ...
            { response ->
                try {
                    ...
                    Toast.makeText(requireContext(),getString(R.string.aracbulunmamakta), Toast.LENGTH_LONG).show()
                ...
                Toast.makeText(requireContext(),getString(R.string.aracbulunmamakta), Toast.LENGTH_LONG).show()
                val alert= Alert()
                alert.showAlert(requireContext(),getString(R.string.uyari),getString(R.string.aracbulunmamakta))
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun AcikKarlarGetir() {
        val today = LocalDate.now()
        val fiveDaysAgo = today.minusDays(10)

        val urlek ="&username="+username+"&uid="+uid+"&baslangic_Tarihi="+fiveDaysAgo+"&bitis_Tarihi="+today+"&kabuldurumu=AÇIK"
    ...
    private fun getUserData(context: Context) {
        ...
        recylerview_acikkartlar.adapter = KartAcikAdapter(newArrayList)
